...
- 更新 `.claude/CLAUDE.md` 的"Wiki 状态"和"覆盖范围"（已摄入 repo 数、页面数、已识别领域等）
- 检查词汇表一致性（见下方）
...
9. **检查新认知**：本次对话是否产生了 wiki 中尚未记录的新理解（跨模块关系、业务规则、接口发现、模式识别）？有则更新相关 wiki 页面
10. **保存有价值的 query**：如果本次问题触发了代码回溯、跨页面关联、或发现了 wiki 的覆盖盲区，将问题和发现路径存入 `.inputs/queries/`（格式：`{描述性名称}.md`，含问题、思考路径、发现、涉及的 repo/模块）
