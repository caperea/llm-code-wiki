...
- **代码导航**：动手读 `.sources/` 之前先读 `references/code-navigation.md`，按操作映射表选工具。简单说：扫描符号/追踪引用/识别实现用 LSP（`documentSymbol`、`workspaceSymbol`、`findReferences`、`goToImplementation` 等），找一段文字才用 grep。
