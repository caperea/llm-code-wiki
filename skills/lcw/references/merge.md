...
**中断恢复**：merge 的所有状态持久化在 `merge-plan.md` 中。如果中断，重新执行 `/lcw merge` 会读取该文件从断点继续。

**独立校验**：`/lcw merge --verify` 不执行合并，只读取 `merge-plan.md` 校验合并完整性。
...
产出：写入 `merge-plan.md` 的"元信息"和"源 wiki 摘要"。
...
产出：写入 `merge-plan.md` 的"合并目标"、"重叠决策"、"词汇表冲突"和完整的"执行清单"（所有 checkbox 初始为 `- [ ]`）。

## 阶段三：展示计划并确认

向用户展示 `merge-plan.md` 的完整内容（此时所有 checkbox 都是 pending）：
...
等用户确认后继续。用户可以调整决策（如"那两个 domain 其实应该合并"），调整后更新 `merge-plan.md` 并重新展示。

## 阶段四：执行合并

按 `merge-plan.md` 中的执行清单逐项执行。**每完成一项立即在文件中标记 `[x]`**。
...
16. **写 log.md**
17. **更新 `merge-plan.md`**：填入"合并目标"的"实际"列

## 阶段五：校验

合并完成后（或通过 `/lcw merge --verify` 独立执行）：

读取 `merge-plan.md`，执行以下检查：
...
合并完成后 `merge-plan.md` 保留在 wiki 根目录，提交到 git——它是这次合并的完整审计记录。
