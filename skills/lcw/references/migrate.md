...
LCW 的目录结构和页面模板会随版本演化（比如新增 `.inputs/` 目录、移除 `concepts/`、模块改为按仓库分子目录）。已有的 wiki 可能基于旧版结构创建，手动调整既繁琐又容易遗漏。migrate 自动完成这个对齐。
...
1. **SCHEMA.md 版本检查**：对比项目中的 `SCHEMA.md` 与 LCW 技能中的 `templates/SCHEMA.md`，检查 `schema_version` 是否一致。版本不同时标记需要升级（这是后续所有页面迁移的前提——页面要按新 schema 调整，必须先有新 schema）
2. 读取 LCW 技能中的目标目录结构（SKILL.md）和最新页面模板（templates/SCHEMA.md）
3. 扫描 wiki 根目录下的所有文件和目录（排除 `.sources/`、`.claude/`、`.git/` 等）
4. 逐项比对，产生分析报告：
...
| 类别 | 说明 |
|------|------|
| SCHEMA.md 需升级 | 项目中的 schema 版本落后于 LCW 最新版本，列出版本差异摘要 |
...
| 缺失的目录 | 当前结构要求但 wiki 中不存在的目录（如 `.inputs/`） |
...
1. **升级 SCHEMA.md**（如版本不同）：
   - 将项目中的旧 `SCHEMA.md` 复制到 `legacy/SCHEMA-v{旧版本号}.md` 保留备份
   - 从 LCW 技能的 `templates/SCHEMA.md` 复制新版本到项目根目录
   - 在计划展示和报告中列出新旧版本的关键差异（新增的页面类型、变更的模板字段、移除的分类等），让用户了解变化
2. 创建缺失的目录（`.inputs/`、`ddd/` 等）
3. 移动文件到正确位置
4. 更新页面 frontmatter 和结构（按新 SCHEMA.md）
5. 将无法归类的文件移入 `legacy/`（保持原始目录结构，如 `legacy/concepts/x.md`）
6. 修复 `[[wikilink]]` 引用（文件移动后链接会断）
7. 重建 `index.md`（以迁移后的实际文件为准）
8. 检查 `glossary.md` 格式是否符合当前 SCHEMA.md 的双视角要求
9. 写 `log.md`
...
- SCHEMA.md 升级情况（旧版本 → 新版本，关键差异摘要，旧版备份位置）
...
`legacy/` 位于 wiki 根目录，提交到 git（内容需要保留供参考）。内部保持文件的原始相对路径结构：
