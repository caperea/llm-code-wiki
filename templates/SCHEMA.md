...
│   └── {context-name}.md           # 聚合根 + 事件 + ACL + 服务 + 跨聚合协调（单文件，按章节组织）
└── evolution/                      # 演进层
    ├── roadmap.md                  # 重构路线图
    ├── transition.md               # 过渡架构
...
DDD 分析状态总览——新读者打开这一个文件就能了解整体进度。每次 DDD 操作完成后自动更新。

必含章节：

- **梳理范围**：纳入的 repo 清单（链接到 `[[ddd/decisions]]`）
- **进度表**：表格列出每个分析步骤的产出文件、状态（未开始 / 进行中 / 已完成 / 已确认）、最后更新时间
...
- **领域服务**：每个服务一行或一节：
  ...
  - 当前实现：代码位置
- **跨聚合协调**（核心域必含，支撑域按需）：每个跨聚合流程一节：
  - 业务流程名称
  - 涉及的聚合根
  - 当前协调模式（大事务 / 隐式调用链 / 事件 / Saga / Process Manager）
  - 目标协调模式（如需变更）
  - 补偿逻辑现状（有 / 缺失 / 部分）
...
- **现状评估**：表格（上下文 / 烟囱程度 / 耦合度 / 模型健康度 / 变更频率 / 问题密度 / 组织就绪度 / 综合优先级）
- **优先级排序**：排序结果及依据说明
